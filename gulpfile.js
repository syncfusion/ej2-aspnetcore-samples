...
gulp.task('testing', function(done){
    var title = adjustTitle('MultiColumn ComboBox', 'data-binding support');
    console.log(title);
    done();
})
...
gulp.task('validate-page-url' , async (done) => {
    const linkToFiles = new Map();
    const IgnoredLinks = [
        'https://www.apple.com/iphone-x/specs/',
        'https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}',
        'https://example.com/image.jpg',
        'https://amazons3.azurewebsites.net/api/AmazonS3Provider/AmazonS3FileOperations',
        'https://services.syncfusion.com/aspnet/production/',
        'https://bi.syncfusion.com/olap/msmdpump.dll',
        'https://www.googleapis.com/calendar/v3/calendars/',
        'https://api.jsonbin.io/v3/b/6912d9ecd0ea881f40e12335',
        'https://www.googletagmanager.com/ns.html'
    ]
    const httpsRegex = /https:\/\/[^\s"'<>]+/g;
    const filePath = fsPath.resolve(__dirname, 'extractedUrls.txt');
    ...
    // validate the the urls
    async function validateLinks() {
        const redirectedEntries = []
        console.log("Validating links.....");
        const file = fs.readFileSync("extractedUrls.txt", "utf-8");
        const links = (file || '')
            .split(/\r?\n/)
            .map(l => clean(l.trim()))
            .filter(Boolean);
        ...
        function clean(url) {
            ...
            // Case 1: remove trailing `);`
            if (url.endsWith(');')) {
                return url.slice(0, -2);
            }

            // Case 2: handle trailing `)`
            if (url.endsWith(')')) {
                const openIndex = url.lastIndexOf('(');
                const closeIndex = url.lastIndexOf(')');

                // If there is NO matching '(' before last ')', it's invalid
        ...
        for (const link of links) {
            if (IgnoredLinks.some(str => link.includes(str))){
                continue;
            }
            try {
                ...
                // Treat redirects specially: collect them and continue
                if(res.redirected){
                    console.log(`⚠️  The link ${link} is Redirected. Please review and correct it.`);
                    redirectedEntries.push({ link, files: Array.from(linkToFiles.get(link) || []) });
                    continue;
                }

                if (res.status == 404 || /404|error|not found/i.test(title) && !res.status==200) {
                    TableFormat();
                    console.log(`❌ The link ${link} is invalid. Please review and correct it.`);
                    console.log("File Paths: " + Array.from(linkToFiles.get(link)))
                    process.exit(1);
                }                
            } catch (err) {
                TableFormat();
        ...
        function TableFormat(){
            // If only redirects were found, print a table with folder name and file path
            if (redirectedEntries.length) {
                console.log('\nRedirected URLs found (folder / file where link appears):');
                const tableData = [["Folder", "File Path", "URL"]];
                redirectedEntries.forEach(entry => {
                    entry.files.forEach(file => {
                        const parts = file.replace(/\\/g, '/').split('/');
                        // derive folder as second/third segment when possible: Common/Pages/Category
                        let folder = parts.length > 2 ? `${parts[1]}` : parts.slice(1, 2).join('/');
                        tableData.push([folder, file, entry.link]);
                    });
                });
                const config = { columns: { 0: { width: 30 }, 1: { width: 60 }, 2: { width: 60 } } };
                console.log(table.table(tableData, config));
            }
        }
    }
    ...
    async function main() {
        let folderNames = (() => {
            try {
                const diff = shelljs.exec(`git diff --name-only origin/${process.env.githubTargetBranch}...origin/${process.env.githubSourceBranch}`, { silent: true });
        ...
        folderNames.forEach(folderName => {
            if (folderName != null) {
                changedDir = glob.sync(`Pages/${folderName}/*.cshtml`);

                const httpsLinks = new Set();

                changedDir.forEach(file => {
                    const content = fs.readFileSync(file, 'utf8');
                    const matches = content.match(httpsRegex);
                    if (!matches) return;
                    matches.forEach(rawLink => {
                        const link = rawLink.trim();
                        httpsLinks.add(`\n`);
                        httpsLinks.add(link);
                        // record origin file for this link
                        const existing = linkToFiles.get(link) || new Set();
                        existing.add(file);
                        linkToFiles.set(link, existing);
                    });
                });
                fs.appendFileSync('extractedUrls.txt', [...httpsLinks].join('\n'));
            }
        })
        await validateLinks();
    }
