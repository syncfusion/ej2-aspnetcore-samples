...
public class LazyLoadGroupWithInfiniteScrollingModel : PageModel
{
    public static List<CustomerDetail> customerdetails = new List<CustomerDetail>();
    public List<CustomerDetail> lazyLoadData { get; set; }
    public void OnGet()
    {
        if (customerdetails.Count == 0)
            BindCustomerDataSource();
        lazyLoadData = customerdetails;
    }
    private void BindCustomerDataSource()
        {
            ...
            for (int x = 0; x < 20000; x++)
            {
                ...
                customerdetails.Add(new CustomerDetail() { OrderID = OrderID + x, CustomerID = customerid[i], CustomerName = customername[j], CustomerAddress = customeraddress[k], ProductName = product[l], ProductID = x, Quantity = quantityperunit[m] });
                i++; j++; k++; l++; m++;
            }
        }
}
