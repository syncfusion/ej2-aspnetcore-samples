...
namespace EJ2CoreSampleBrowser.Controllers.PDF
{
    public partial class PdfController : Controller
    {
        ...
        [HttpPost]
        public ActionResult WatermarkPDF(string Browser, string Stamptext, string imageWatermark, float transparency, IFormFile file, IFormFile imageFile)
        {
            PdfLoadedDocument ldoc = null;
            Stream fileStream = GetInputDocument(file);

            if ((imageWatermark == " Watermark" && imageFile != null && imageFile.Length > 0) || imageWatermark == null && Stamptext != null)
            {
            ...
            {
                ViewBag.lab = "Please select a valid image file or enter text to add a watermark.";
                return View();
            }
