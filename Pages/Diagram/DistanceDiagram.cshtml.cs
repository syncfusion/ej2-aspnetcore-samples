...
{
    public class DistanceDiagramModel : PageModel
    {
        ...
        public List<DropDownListFieldSettings> UnitOptions { get; set; }
        public string SelectedUnit { get; set; } = "in";
        public DiagramRulerSettings RulerSettings { get; set; } = new DiagramRulerSettings();
        ...
        public void OnGet()
        {
            ...
            UnitOptions = new List<DropDownListFieldSettings>();
            {
                UnitOptions.Add(new DropDownListFieldSettings{ Text = "Inches (in)", Value = "in" });
                UnitOptions.Add(new DropDownListFieldSettings{ Text = "Feet (ft)", Value = "ft" });
                UnitOptions.Add(new DropDownListFieldSettings{ Text = "Centimeters (cm)", Value = "cm" });
            };
            SelectedUnit = "in";

            // Create the floor plan layout
            CreateFloorPlanLayout();
            
            // Create measurement connectors
            CreateMeasurementConnectors();

            // Configure ruler settings
            RulerSettings = new DiagramRulerSettings
            {
                ShowRulers = true
            };
        }

        private void CreateFloorPlanLayout()
        {
            // Create outer walls
            ...
            CreateWallConnector("Separator2", 628, 150, 628, 310, "#333333", 5);
            ...
            // Create furniture items as nodes
            CreateFurnitureItems();
        }

        private void CreateWallConnector(string id, int startX, int startY, int endX, int endY, string color, double thickness)
        {
            var constraints = id.Contains("WallOuter")
                ? ConnectorConstraints.None
                : (ConnectorConstraints.Default|ConnectorConstraints.ReadOnly) & ~(ConnectorConstraints.Drag | ConnectorConstraints.DragSourceEnd);
            var wallConnector = new DiagramConnector
            {
                Id = id,
                ...
                Style = new DiagramStrokeStyle { StrokeColor = color, StrokeWidth = thickness },
        ...
        private void CreateMeasurementConnectors()
        {
            // Create measurement connectors for separators
            var separators = new[] { "Separator1", "Separator2", "Separator3", "Separator4", "HallSeparator", "RestroomSeparator" };
            
            foreach (var separatorId in separators)
            {
                var separator = Connectors.FirstOrDefault(c => c.Id == separatorId);
                if (separator != null)
                {
                    CreateMeasurementConnectorForSeparator(separator);
                }
            }

            // Create overall measurement connectors
            CreateOverallMeasurementConnectors();
        }

        private void CreateMeasurementConnectorForSeparator(DiagramConnector separator)
        {
            double distance, startX, startY, endX, endY;
            bool isHorizontal;
            ...
            switch (separator.Id)
            {
                case "Separator1":
                    ...
                    startX = separator.SourcePoint.X;
                    endX = separator.TargetPoint.X;
                    startY = endY = 670; // wallOuterBottom.Y + 60
                    isHorizontal = true;
                    break;
                ...
                case "Separator3":
                    ...
                    startX = separator.SourcePoint.X;
                    endX = separator.TargetPoint.X;
                    startY = endY = 640; // wallOuterBottom.Y + 30
                    isHorizontal = true;
                    break;

                case "HallSeparator":
                    distance = Math.Abs(separator.TargetPoint.Y - separator.SourcePoint.Y);
                    startY = separator.SourcePoint.Y;
                    endY = separator.TargetPoint.Y;
                    startX = endX = 260;
                    isHorizontal = false;
                    break;
                ...
                case "Separator4":
                    ...
                    startX = separator.SourcePoint.X;
                    endX = separator.TargetPoint.X;
                    startY = endY = 130; // wallOuterTop.Y - 20
                    isHorizontal = true;
                    break;
                ...
                case "Separator2":
                    ...
                    startY = separator.SourcePoint.Y;
                    endY = separator.TargetPoint.Y;
                    startX = endX = 940; // wallOuterRight.X + 40
                    isHorizontal = false;
                    break;
                ...
                case "RestroomSeparator":
                    ...
                    startY = separator.SourcePoint.Y;
                    endY = separator.TargetPoint.Y;
                    startX = endX = 980; // wallOuterRight.X + 80
                    isHorizontal = false;
                    break;
            ...
            var dimensionText = FormatMeasurement(ConvertPixelsToSelectedUnit(distance));
            CreateDimensionConnector(separator.Id + "Measurement", startX, startY, endX, endY, dimensionText, isHorizontal);
        }

        private void CreateOverallMeasurementConnectors()
        {
            // Total Width Measurement
            var totalWidth = 602; // 901 - 299
            var widthText = FormatMeasurement(ConvertPixelsToSelectedUnit(totalWidth));
            CreateDimensionConnector("TotalWidthMeasurement", 300, 100, 900, 100, widthText, true);

            // Total Height Measurement  
            var totalHeight = 460; // 610 - 150
            var heightText = FormatMeasurement(ConvertPixelsToSelectedUnit(totalHeight));
            CreateDimensionConnector("TotalHeightMeasurement", 240, 150, 240, 610, heightText, false);
        }
        ...
        {
            var dimensionLine = new DiagramConnector
            {
                Id = id,
                SourcePoint = new DiagramPoint() { X = startX, Y = startY },
                TargetPoint = new DiagramPoint() { X = endX, Y = endY },
                ...
                Style = new DiagramStrokeStyle { StrokeColor = "#0066CC", StrokeWidth = 2 },
                SourceDecorator = new DiagramDecorator
                {
                    Shape = DecoratorShapes.Arrow,
                    Style = new DiagramShapeStyle { Fill = "#0066CC", StrokeColor = "#0066CC" }
                },
        ...
        {
            var extensionLine = new DiagramConnector
            {
                Id = id,
                SourcePoint = new DiagramPoint() { X = startX, Y = startY },
                TargetPoint = new DiagramPoint() { X = endX, Y = endY },
                ...
                SourceDecorator = new DiagramDecorator { Shape = DecoratorShapes.None },
                TargetDecorator = new DiagramDecorator { Shape = DecoratorShapes.None },
                Style = new DiagramStrokeStyle { StrokeColor = "#0066CC", StrokeWidth = 1 }
            };
