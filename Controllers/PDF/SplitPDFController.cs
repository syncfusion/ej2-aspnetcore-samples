...
namespace EJ2CoreSampleBrowser.Controllers.PDF
{
    public partial class PdfController : Controller
    {
        ...
        [HttpPost]
        public ActionResult SplitPDF(string Browser, string splitOption, int startPageNumber, int endPageNumber, int fileCount, int pageNoCount, IFormFile file)
        {          
           result = null;
                ...
                if(fileCount != 0)
                {
                    if (fileCount <= pageCount)
                    {
                        ...
                        ldoc.DocumentSplitEvent += (object sender, PdfDocumentSplitEventArgs args) =>
                        {
                            ...
                            zipArchive.AddItem($"split{splitRanges[index, 0] + 1}-{splitRanges[index, 1] + 1}.pdf", stream, false, (Syncfusion.Compression.FileAttributes)FileAttributes.Normal);
                            index++;
                        };
                ...
                if(pageNoCount != 0)
                {
                    if (pageNoCount <= pageCount)
                    {
                        ...
                        ldoc.DocumentSplitEvent += (object sender, PdfDocumentSplitEventArgs args) =>
                        {
                            MemoryStream stream = new MemoryStream();
                            args.PdfDocumentData.CopyTo(stream);
                            zipArchive.AddItem($"split{index}.pdf", stream, false, (Syncfusion.Compression.FileAttributes)FileAttributes.Normal);
                            index++;
                        };
        ...
        private Stream GetSplitPDFDocument(IFormFile file)
        {
            if (file != null)
            {
                ...
                {
                    ViewBag.Message = string.Format("Please choose a valid PDF document to split");
                    return null;
                }
            }
            else
