...
public class LazyLoadGroupWithVirtualScrollingModel : PageModel
{
    public static List<CustomerDetails> customerdata = new List<CustomerDetails>();
    public List<CustomerDetails> lazyLoadData { get; set; }

    public void OnGet()
    {
        if (customerdata.Count == 0)
            BindData();
        lazyLoadData = customerdata;
    }
    private void BindData()
        {
            ...
            for (int x = 0; x < 20000; x++)
            {
                ...
                customerdata.Add(new CustomerDetails() { OrderID = OrderID + x, CustomerID = customerid[i], CustomerName = customername[j], CustomerAddress = customeraddress[k], ProductName = product[l], ProductID = x, Quantity = quantityperunit[m] });
                i++; j++; k++; l++; m++;
            }
        }
}
