...
namespace EJ2CoreSampleBrowser.Controllers
{
    ...
    {
        public IActionResult AnimationConverter()
        {
            ViewBag.ImagesCount = 0;
            ConvertToImage();
            return View();
        }
        [HttpPost]
        public ActionResult AnimationConverter(string button, string view)
        {
            ViewBag.ImagesCount = 0;
            if (button == null)
                return View();
        ...
        private void ConvertToImage()
        {
            ...
            if (fileStream != null)
            {
                ...
                ViewBag.ImagesCount = imagesFolder.GetFiles().Length;
            }
        }
        ...
        private Stream GetAnimationPresentation()
        {
            if (Request.ContentType != null && Request.Form.Files != null && Request.Form.Files.Count != 0)
            {
                ...
                if (extension == ".pptx")
                {
                    ...
                    ViewBag.FileName = Request.Form.Files[0].FileName;
                    return stream;
                }
                else
                {
                    ViewBag.Message = string.Format("Please choose PowerPoint Presentation document (PPTX) to convert as Image");
                    return null;
                }
            }
            else
