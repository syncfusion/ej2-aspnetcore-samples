...
namespace EJ2CoreSampleBrowser.Controllers.Word
{
    ...
    {
        public IActionResult WordToPDFA(string button)
        {
            if (button == null)
                return View();
            ...
            if(stream != null)
            {
                try
                {
                    string output = (Request.Form.Files != null && Request.Form.Files.Count != 0) ? Path.GetFileNameWithoutExtension(Request.Form.Files[0].FileName) : "WordtoPDF_Pdf_A3A";
