...
public class TableFeatures : PageModel
{
    public void OnGet()
    {
    ...
    public TableFeatures(IWebHostEnvironment hostingEnvironment)
    {
    ...
    public ActionResult OnPost(string InsideBrowser)
    {
        ...
        PdfPageTemplateElement footer = new PdfPageTemplateElement(
            new RectangleF(new PointF(0, document.PageSettings.Height - 40),
                new SizeF(document.PageSettings.Width, 40)));
        footer.Graphics.DrawRectangle(new PdfSolidBrush(new PdfColor(77, 77, 77)), footer.Bounds);
        footer.Graphics.DrawString("http://www.syncfusion.com", font, grayBrush,
            new PointF(footer.Width - (footer.Width / 4), 15));
        ...
        page.Graphics.DrawRectangle(orangeBrush,
            new RectangleF(PointF.Empty, new SizeF(page.Graphics.ClientSize.Width, margin)));

        //Draw the text to the PDF page
        page.Graphics.DrawString("Essential Studio File Formats Edition", bigFont, PdfBrushes.White,
            new PointF(10, 10));
        ...
        PdfLayoutResult result = pdfLightTable.Draw(page,
            new RectangleF(new PointF(margin, 70),
                new SizeF(page.Graphics.ClientSize.Width - (2 * margin), page.Graphics.ClientSize.Height - margin)));

        # endregion

        page.Graphics.DrawString("What You Get with Syncfusion", bigFont, orangeBrush,
            new PointF(margin, result.Bounds.Bottom + 50));
        ...
        pdfGrid.Draw(page,
            new RectangleF(new PointF(margin, result.Bounds.Bottom + 75),
                new SizeF(page.Graphics.ClientSize.Width - (2 * margin), page.Graphics.ClientSize.Height - margin)),
            gridLayoutFormat);
    ...
    void pdfLightTable_BeginCellLayout(object sender, BeginCellLayoutEventArgs args)
    {
        if (args.RowIndex > -1 && args.CellIndex > -1)
        {
            ...
            if (args.Value.Contains("http"))
            {
                ...
                textLink.DrawTextWebLink(args.Graphics,
                    new PointF(args.Bounds.X + 2 * args.Bounds.Width / 3, args.Bounds.Y));
            }
    ...
    internal sealed class DataProvider
    {
        ...
        public static IEnumerable<Report> GetReport(string webRootPath)
        {
            string dataPath = webRootPath + @"/PDF/";
            //Read the file
            FileStream xmlStream = new FileStream(dataPath + "Report.xml", FileMode.Open, FileAccess.Read,
                FileShare.ReadWrite);
